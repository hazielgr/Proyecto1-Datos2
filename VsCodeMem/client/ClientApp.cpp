...
void ClientApp::sendList(LinkedList<string> listToSend){
    Node<string>* temp = listToSend.head;
    while(temp!= nullptr){
        Data = jsonSerialize::enCode(temp);
        sendData(sock);
        temp = temp->getNext();
    }
}

void ClientApp::sendList(LinkedList<bool> listToSend){
    Node<bool>* temp = listToSend.head;
    while(temp!= nullptr){
        Data = jsonSerialize::enCode(temp);
        sendData(sock);
        temp = temp->getNext();
...
void ClientApp::sendList(LinkedList<int> listToSend){
    Node<int>* temp = listToSend.head;
    while(temp!= nullptr){
        Data = jsonSerialize::enCode(temp);
        sendData(sock);
        temp = temp->getNext();
